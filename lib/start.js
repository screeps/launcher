...
module.exports = async function start(_opts, output, nodeBin) {
    ...
    const nodeExecutable = nodeBin || process.execPath;
    ...
    async function _startProcess(name, execPath, env, restartInterval = 0) {
        ...
        result.processes[name] = cp.spawn(nodeExecutable, ['--no-node-snapshot', path.resolve(execPath)], {
