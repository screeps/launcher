...
var libmime = module.exports = {
    ...
    /**
     * Checks if a multi line string contains lines longer than the selected value.
     *
    ...
    buildHeaderParam: function (key, data, maxLength, fromCharset) {
        ...
        if (libmime.isPlainText(data)) {
            ...
            if (/[\uD800-\uDBFF]/.test(encodedStr)) {
                // string contains surrogate pairs, so normalize it to an array of bytes
                encodedStrArr = [];
                for (i = 0, len = encodedStr.length; i < len; i++) {
                    chr = encodedStr.charAt(i);
...
function safeEncodeURIComponent(str) {
    ...
    // ensure chars that are not handled by encodeURIComponent are converted as well
